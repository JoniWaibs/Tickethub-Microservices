...
describe('get ticket controller methods', () => {
  ...
  test('returns 400 when ticket does not contain a valid id type', async () => {
    const response = await request(server).get(`${baseUrl}/not-valid-id`);
  ...
  test('returns 404 when ticket does not exists', async () => {
    const response = await request(server).get(`${baseUrl}/62b7c6adf9474e78ff36abb1`);
  ...
  test('retrieves a ticket by id', async () => {
     ...
     */
    const ticketResponse = await request(server).get(`${baseUrl}/${response.body.id}`).set('Cookie', cookie).send();
